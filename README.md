...
## Usage
Once launched, CSharpGOL automatically determines the dimensions (height and width) of the terminal instance it was in which it was run and generates a grid to match these dimensions. The individual cells in this starting grid are randomly set as live or dead, to create the initial state for the simulation.
...
Each generation, the state of the current grid is converted to a display string and printed to the console. A rudimentary timing mechanism is used to trigger an update of the display approximately every 100ms, to show show the evolution of the simulation over each successive generation and to ensure a relatively uniform "frame rate" (in order to prevent visual glitches/flashing). The number for the current generation is centered and displayed on a header row at the top of the terminal screen.
